...
namespace UFlow.Addon.EntityStates.Core.Runtime.Systems {
    public abstract class BaseGlobalStateDeltaSystem<TState> : BaseSetIterationCallbackDeltaSystem
        where TState : IEcsState {
        protected EntityStateCommandBuffer StateCommandBuffer { get; }

        public BaseGlobalStateDeltaSystem(in World world) : base(in world, world.BuildQuery().With<TState>()) =>
